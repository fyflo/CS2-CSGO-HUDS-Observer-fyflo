...
socket.element.addEventListener("projectiles", (event) => {
  ...
  for (let projectile of projectiles) {
    ...
    if (!projectileElement) {
      ...
      trailElement = document.createElementNS(
      ...
      // Полосы гранат
      trailElement.setAttributeNS(
        null,
        "stroke-width",
        global.mapData.resolution * 0.2
      );
    ...
    if (projectile.type == "frag") {
      ...
      for (
        let i = trailParts.length - 1;
        i > 2 && trailParts.length - i < 5 * 3;
        i -= 3
      ) {
        // If the nade moved in this frame, it is not stationary
        if (
          trailParts[i] != trailParts[i - 3] ||
          trailParts[i - 1] != trailParts[i - 4]
        ) {
          isStationary = false;
          break;
        }
    ...
    trailElement.setAttributeNS(
      null,
      "d",
      trailElement.getAttributeNS(null, "d") + " L " + posX + " " + (100 - posY)
    );
  }

  // For each projectile element on the map
  for (let projectileElement of document.getElementById("projectiles")
    .children) {
    ...
    if (!activeProjectiles.includes(projectileElement.id)) {
      ...
      let trailElement = document.getElementById(
        "trail" + projectileElement.id
      );
      if (trailElement)
        document.getElementById("trails").removeChild(trailElement);
