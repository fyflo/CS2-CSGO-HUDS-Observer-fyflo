const COLOR_CT = "rgba(10,81,127,1)";
const COLOR_T = "rgba(143,11,7,1)";
const COLOR_NEW_CT = "rgba(10,81,127,1)";
const COLOR_NEW_T = "rgba(143,11,7,1.0)";
...
function updateStateOver(phase, round, previously) {
  if (phase) {
    ...
    if (round.win_team == "CT") {
      if (teams.left.side == "ct") {
        // * CT alert on Left
        showAlertSlide("#left_team", COLOR_WHITE, "WINS THE ROUND");
      } else {
        // * CT alert on Right
        showAlertSlide("#right_team", COLOR_WHITE, "WINS THE ROUND");
      }
  ...
  if (phase) {
    if (round.bomb == "planted") {
      ...
      if (checkPrev(previously, "live")) {
        ...
        showMiddleAlert(COLOR_NEW_T, COLOR_NEW_T, "BOMB PLANTED", COLOR_NEW_T, "/files/img/elements/icon_c4_default.png");
...
function fillObserved(obs) {
  ...
  if (obs.team == "CT") {
    if (teams.left.side == "ct") {
      ...
      flag = teams.left.flag;
    } else {
      _img = teams.right.logo;
      flag = teams.right.flag;
    }
  } else if (obs.team == "T") {
    if (teams.left.side == "t") {
      _img = teams.left.logo;
  ...
  // Team Logo and Flags
	...
	$("#obs_team_img").attr("src", "/files/img/flags-50/" + flag + ".png");
  if (disp_player_flags) {
	  

//	console.log(teams.left.flag)
	//console.log(teams.right.flag)
...
function fillPlayer(player, nr, side, observed, phase, previously) {
  ...
  let gradient_single = "linear-gradient(to " + side.substr(8) + ", rgba(152,152,152,19) " + (100 - stats.health) + "%, " + alt_health_color + " " + (100 - stats.health) + "%)";
...
function showMiddleAlert(pole_left_color, pole_right_color, text, text_color, icon = null) {
  ...
  $("#alert_middle #pole_2_middle").css("background-color", pole_right_color);
	if(icon == null){
	  $("#alert_middle #alert_text_middle")
		...
		$("#alert_middle #alert_text_middle")
		.html("<img src='"+icon+"'  style='width: 40px; height: auto;position: absolute; left: 40px;'}>  &nbsp; &nbsp;"+ text)
		.css("color", text_color);
	}
