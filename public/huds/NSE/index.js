...
function fillPlayer(player,nr, side, max){
    ...
    let health_color = statistics.health <= 20 ? "#9d1e1e" : team == "ct" ? "#349ec9":"#c9a434";
    ...
    $top.find("#hp_p").removeClass("dead").addClass(statistics.health == 0 ? "dead" : "");
    ...
    $bottom.find(".moneys").removeClass("dead").addClass(statistics.health == 0 ? "dead" : "");
    ...
    if(statistics.round_kills > 0){
        ...
        $bottom.find("#weapon_icon").prepend($("<img />").attr("src", "/files/img/elements/death.png").addClass("death").css("float", side)).prepend($("<div></div>").text(statistics.round_kills).css(img_css));
    }
    ...
    if (phase) {
        ...
        if (phase.phase == "bomb" || phase.phase == "defuse") {
            ...
            if (phase.phase == "defuse") {
                ...
                $("#defuse_bar").css("width", 365 * (parseFloat(phase.phase_ends_in) / longd) + "px");
